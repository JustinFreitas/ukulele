...
class SkipCommand : Command("skip", "s") {
    ...
    private fun CommandContext.toggleShowQueueOnSkip() {
        ...
        val showQueueOnSkipMessage =
            when (player.showQueueOnSkip) {
    ...
    private fun listTracksInQueue(tracks: List<AudioTrack>) =
        buildString {
            tracks.forEachIndexed { i, t ->
                appendLine(
                    "`[${i + 1}]` **${t.info.title}** `[${if (t.info.isStream) "Live" else TextUtils.humanReadableTime(t.duration)}]`",
                )
            }
        }
